...
def ft_1lead_ssl(device, n_classes):
    model = ResNet1D(
        in_channels=1,
        base_filters=64, 
        kernel_size=16,
...
def ft_1lead_ECGFounder(device, n_classes):
  model = Net1D(
      in_channels=1, 
      base_filters=64, #32 64
      ratio=1, 
  ...
  checkpoint = torch.load('/hot_data/lijun/code/partners_ecg-master/device_checkpoint_10000_0.8692.pth', map_location=device)
  state_dict = checkpoint['state_dict']
...
def ft_1lead_ramdom(device, n_classes):
  model = Net1D(
      in_channels=1, 
      base_filters=64, #32 64
      ratio=1, 
